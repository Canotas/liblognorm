...
# SEC-3 regression: extension value ending in a single backslash must
# not cause an out-of-bounds read or crash; the lone backslash is an
# invalid escape so the whole message must be rejected
execute 'CEF:0|Vendor|Product|Version|Signature ID|some name|Severity| name=value\'
assert_output_json_eq '{ "originalmsg": "CEF:0|Vendor|Product|Version|Signature ID|some name|Severity| name=value\\", "unparsed-data": "CEF:0|Vendor|Product|Version|Signature ID|some name|Severity| name=value\\" }'
